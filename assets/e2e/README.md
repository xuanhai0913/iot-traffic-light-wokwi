...
> Thư mục này chứa ảnh chụp màn hình và log MQTT capture cho `report/e2e_test_results.md`.
...
4. Copy file vào thư mục `assets/e2e/`.
...
python3 scripts/mqtt-sniff.py --seconds 60 \
  > assets/e2e/mqtt_capture_live.log 2>&1
```
...
python3 scripts/mqtt-sniff.py --seconds 10 \
  ...
  > assets/e2e/mqtt_capture_auto.log 2>&1 &
...
ls -la assets/e2e/*.png

# Đếm số lệnh SET_* trong mọi log
grep -h "SET_" assets/e2e/mqtt_capture_*.log | sort -u
```
