...
| **Có ghi nhận kiểm thử** | File trạng thái của dự án ghi nhận đã chạy thành công, nhưng lượt rà soát tài liệu này không chạy lại để tránh ghi ngoài phạm vi cập nhật tài liệu hiện tại. |
...
| Ảnh/video Wokwi | `docs/evidence/wokwi/wokwi_*.png` | Đã có ảnh compile/run và 5 ảnh mode; chưa có video cuối |
...
Chi tiết thao tác và checklist nằm tại [demo_dien_thoai.md](../demo/demo_dien_thoai.md).
...
- Đã có ảnh Wokwi thực trong `docs/evidence/wokwi/`; chưa có video/GIF cuối.
- Chưa có report PDF và slide hoàn chỉnh trong `docs/report/` hoặc thư mục slide riêng.
...
Ma trận chi tiết theo FR/ER/NFR nằm tại [ma_tran_tieu_chi_iot.md](../design/ma_tran_tieu_chi_iot.md).
