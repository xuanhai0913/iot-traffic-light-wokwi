...
> Tài liệu này là bằng chứng thực nghiệm cho báo cáo kỹ thuật (`docs/report/bao_cao_ky_thuat.md`) và slide thuyết trình cuối kỳ.
...
| MQTT capture | `scripts/mqtt-sniff.py` (đính kèm), output lưu tại `docs/evidence/e2e/mqtt_capture_*.log` |
...
- **Bằng chứng:** Ảnh chụp `docs/evidence/e2e/tc02_auto_connect.png`.
...
- **Bằng chứng:** Ảnh `docs/evidence/e2e/tc04_auto_result.png` + `mqtt_capture_auto.log`:
...
- **Thực tế:** PASS.
- **Bằng chứng:**
  - Ảnh: `docs/evidence/e2e/tc06_priority_ns_confirm.png`
...
- **Thực tế:** PASS. Ví dụ gửi `SET_EMERGENCY`:
  ...
  ```
- **Bằng chứng:** Ảnh `docs/evidence/e2e/tc12_wokwi_serial.png`.
...
- **Thực tế:** PASS. Capture mẫu (`docs/evidence/e2e/mqtt_capture_acks.log`):
  ```text
...
- **Bước thực hiện:** Duyệt qua 5 tab của Flutter web và toàn bộ các màn hình chính trong `flutter_app/lib/main.dart`.
- **Mong đợi:** 100% chuỗi người dùng nhìn thấy có dấu Tiếng Việt đầy đủ.
- **Thực tế:** PASS. Đối chiếu các chuỗi người dùng chính trong `flutter_app/lib/main.dart`:
...
- **Bằng chứng:** Ảnh `docs/evidence/e2e/tc14_vietnamese_ui.png` + lệnh grep:
  ```bash
...
Capture minh chứng cho `SET_AUTO` (lưu tại `docs/evidence/e2e/mqtt_capture_auto.log`):
...
| Hạn chế | Khuyến nghị |
|---|---|
| Chưa có auth — bất kỳ ai biết IP backend đều gửi lệnh được. | Bổ sung API key đơn giản ở header `X-Api-Key` cho môi trường production (xem `docs/report/deployment_recommendation.md`). |
