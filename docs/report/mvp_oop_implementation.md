...
> Phụ lục này bám theo mã nguồn ngày 28/06/2026. Nội dung mô tả OOP thực tế, không suy rộng thành clean architecture hoặc production architecture.
...
Mẫu thiết kế chính là **encapsulation**, **composition**, phân tách trách nhiệm, dependency injection và strategy cho các mode firmware. Inheritance/polymorphism có xuất hiện trong firmware qua `IModeStrategy`, nhưng hệ thống vẫn ở mức MVP chứ chưa phải clean architecture hoàn chỉnh.
...
classDiagram
    ...
    IntersectionController o-- IModeStrategy
...
| `IModeStrategy` + mode classes | Tách hành vi AUTO/NIGHT/PRIORITY/EMERGENCY/MAINTENANCE | Strategy / polymorphism mức MVP |
...
- Đã có strategy cho mode, nhưng vẫn tập trung trong một file lớn nên chưa phải thiết kế module hóa sâu.
...
- API URL đã được persist qua `SharedPreferences` trong Flutter app.
