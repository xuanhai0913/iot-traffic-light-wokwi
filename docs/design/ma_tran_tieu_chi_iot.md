...
| FR-08 | Ảnh/video/GIF chứng minh mô phỏng chạy | Đạt ảnh, chưa có video | Đã có 5 ảnh mode và ảnh compile/run trong `docs/evidence/wokwi/`; video cuối chưa quay |
...
| ER-07 | Mobile app/web | Đạt implementation | Flutter source và APK artifact hiện có |
...
| ID | Tiêu chí | Trạng thái | Ghi chú |
|---|---|---|---|
| NFR-01 | Wokwi chạy ổn, không lỗi compile | Đạt evidence ảnh | Wokwi UI compile/run đã xác nhận; xem `docs/evidence/wokwi/wokwi_compile_run.png` |
...
| Slide hoàn chỉnh | Chưa tìm thấy file slide trong repo |
