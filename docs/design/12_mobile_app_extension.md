...
Khả thi và phù hợp với đề tài IoT. Ở trạng thái hiện tại, repo giữ một lớp UI chính là `flutter_app/`, có thể build cho điện thoại hoặc web; lớp này gọi backend C# thật. Ở phía thiết bị mô phỏng, backend publish command qua MQTT broker để ESP32/Wokwi nhận lệnh và gửi ACK/status ngược lại.

Không nên nói app mobile điều khiển trực tiếp Wokwi. Cách nói đúng hơn là: app/web điều khiển backend, backend điều phối MQTT, còn Wokwi mô phỏng lớp thiết bị.
...
- Flutter lưu cấu hình `API base URL` qua `SharedPreferences`.
...
- Flutter app và Flutter web build gọi backend C# thật.
...
- Flutter app/web là lớp vận hành.
