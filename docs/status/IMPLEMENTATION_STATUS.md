...
- Operator app source: `flutter_app/`, targeting Android, web, Windows, and iOS while calling the same C# backend API.
...
- Flutter UI displays the current mode, phase, countdown, signal state, and command history from the backend.
- Flutter UI now includes operator dashboard panels: metrics, four approach signals, signal heads, phase plan, mode priority, roads, history, and logs.
...
- Flutter UI includes road/approach management for a multi-road intersection model.
...
```text
Flutter app/web -> C# API -> broker.hivemq.com -> ESP32/Wokwi
ESP32/Wokwi -> broker.hivemq.com -> C# API -> Flutter app/web
```
...
- Wokwi evidence screenshots are saved in `docs/evidence/wokwi/`.
