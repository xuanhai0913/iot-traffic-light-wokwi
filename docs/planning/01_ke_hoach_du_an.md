...
Nhóm không có phần cứng thật, vì vậy dự án ưu tiên mô phỏng đầy đủ trên Wokwi. Ở trạng thái hiện tại, repo không chỉ có mô phỏng Wokwi mà còn có backend C# ASP.NET Core, SQLite, MQTT bridge và Flutter app để thể hiện đầy đủ lớp vận hành IoT.

Một điểm quan trọng khi làm bài lớn là nên thể hiện được nhiều kiến thức đã học trong môn và các học phần liên quan. Vì vậy ngoài mô phỏng Wokwi, bản hiện tại còn triển khai thật phần thiết kế CSDL, database lưu lịch sử điều khiển, OOP trong firmware, backend/API, Flutter app/web và luồng MQTT end-to-end.
...
- App/mobile: đã có Flutter app source và có thể build web; luồng điều khiển dùng HTTP + MQTT.
